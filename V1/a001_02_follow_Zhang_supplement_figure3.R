...
file = read.csv(paste0(path,'Figure1_contr_rate_noloss.csv')) 
...
tiff(
  filename = paste(path_pic,"a001_02_follow_Zhang_supplement_figure3.tiff", sep = ""),
  width = pic_width,
  height = pic_height,
  units = "cm",
  pointsize = 10,
...
region_name = c('Asia (AS)', 'Indian Ocean (IO)', 'North Africa (NAF)', 'North Atlantic Ocean (NATO)',
...
for (ipic in 1:13) {
  ...
  if(ipic==2){
    ...
    ytext = ylim
    resource_con_monthly = resource_con_monthly
  }
  if(ipic==3){
    y_min = 0;y_max = 50
    row_lines = seq(y_min,y_max,5)
  ...
  if(ipic==6){
    ...
    ytext = c('0.0','0.5','1.0','1.5','2.0','2.5')
  }
  if(ipic==7){
    y_min = 0;y_max = 6
