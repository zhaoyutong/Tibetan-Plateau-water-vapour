...
evap_all_en = ncvar_get(file,'e')[indexlon1:indexlon2,indexlat1:indexlat2,stime:etime]*1000#m to mm
nc_close(file)
###
...
for (iregion in 1:nregions) {
  ...
  evap_tibet_ave = area_average(longitude,latitude,used_shp)
  evap_region_mask = array(NaN,dim = dim(evap_all_en))
  ntimes = dim(evap_all_en)[3]
  for (iyear in 1:ntimes) {
    evap_region_mask[,,iyear] = evap_all_en[,,iyear]*evap_tibet_ave
  }
  evap_region_ave = apply(evap_region_mask,c(3),mean,na.rm=T)
  path_out = 'I:/Nature_comments/path_out/'
  save(evap_region_ave,
            file = paste0(path_out,'c001_02_ERA5_evapipitation_',riverbasin[iregion],'.Rdata'))
